#!/usr/bin/env bash
# ==============================================================================
# Dell Latitude 7210 2-in-1: Complete Hyprland Tablet Suite Installer
...
echo -e "${BLUE}[1/5] Checking runtime dependencies...${RESET}"
MISSING_PKGS=()
...
if [ ${#MISSING_PKGS[@]} -ne 0 ]; then
    ...
    echo -e "On Arch Linux, install with: sudo pacman -S brightnessctl libpulse wireplumber python python-gobject gtk-layer-shell iio-sensor-proxy"
fi

echo -e "${BLUE}[2/5] Installing tablet helper scripts to ~/.local/bin...${RESET}"
...
echo -e "${BLUE}[3/5] Installing systemd user service (auto-rotate)...${RESET}"
...
echo -e "${BLUE}[4/5] Installing backlight udev rules...${RESET}"
...
echo -e "${BLUE}[5/5] Waybar Touch Configuration...${RESET}"
